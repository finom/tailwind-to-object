...
var getStyles = function () {
    ...
    var entries = CheatSheet.map(function (_a) {
        var content = _a.content;
        return content.map(function (_a) {
            var table = _a.table;
            return table.map(function (list) {
                var twStyles = {};
                var selector = list.length === 3 ? list[0] : list[1];
                var styleStr = list.length === 3 ? list[1] : list[2];
                ...
                return [selector.slice(1), twStyles];
            });
        });
    }).flat(2);
    return Object.fromEntries(entries);
};
function tailwindToObject(selector) {
    if (!selector)
        return {};
    if (!styles) {
        styles = getStyles();
    ...
    selector
        ...
        if (s.includes('[')) {
            var property = s.split('-[')[0].replace('.', '');
            var properyValue = (_b = s.match(/(?<=\[)[^\][]*(?=])/g)) === null || _b === void 0 ? void 0 : _b[0];
            if (arbitrarySupportedClasses[property]) {
                Object.assign(styleAttr, (_a = {},
                    _a[camelCase(arbitrarySupportedClasses[property])] = properyValue + (important ? ' !important' : ''),
                    _a));
            }
            else {
                throw new Error("No style found for selector: ".concat(s));
            }
        }
        else {
            var style = styles[s];
            ...
            Object.assign(styleAttr, mapValues(style, function (value) { return (important ? "".concat(value, " !important") : value); }));
        }
    });
    return styleAttr;
}
exports.default = tailwindToObject;
